"""
`agentinc.toml` — what an agent depends on, and what a tool offers.
...
class AgentManifest(BaseModel):
    """The `agentinc.toml` sitting next to an agent's entry point."""
...
class ToolManifest(BaseModel):
    """The `agentinc.toml` sitting next to a tool's implementation."""
...
def load_agent_manifest(directory: Path | str) -> AgentManifest:
    """Read and validate the `agentinc.toml` in an agent directory."""
...
def load_tool_manifest(directory: Path | str) -> ToolManifest:
    """Read and validate the `agentinc.toml` in a tool directory."""
    path = Path(directory) / MANIFEST_NAME
    raw = _read_toml(path)
