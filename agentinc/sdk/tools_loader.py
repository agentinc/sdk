...
TOOLS_ROOT_ENV = "AGENTINC_TOOLS_ROOT"
...
_MOUNTED_PACKAGE = "agentinc_mounted_tools"
...
class MountedToolResolver:
    ...
    Layout, one directory per tool, named from the slug::
        ...
        <root>/agentinc/echo/agentinc.toml   # the tool manifest
        <root>/agentinc/echo/__init__.py     # its code
...
def _import_from_directory(slug: str, directory: Path, entry: str) -> ModuleType:
    ...
    be *installed* — `agentinc_tools.echo` — while the mount holds only that
    ...
    does ship `agentinc_tools/echo.py` inside its artifact resolves to itself,
    ...
    # directory, so `agentinc_tools/echo/__init__.py` arrives as `__init__.py`
...
def _load_module(slug: str, file: Path, directory: Path) -> ModuleType:
    ...
    Named `agentinc_mounted_tools.<scope>_<name>` rather than the module path
...
def _ensure_mounted_package() -> None:
    ...
    current module, so a module named ``agentinc_mounted_tools.acme_calc``
    with no ``agentinc_mounted_tools`` in ``sys.modules`` fails with
    ``No module named 'agentinc_mounted_tools'`` — raised from inside the
