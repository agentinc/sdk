...
from agentinc.sdk.manifest import (
    ManifestError,
    load_agent_manifest,
    load_tool_manifest,
)
...
def _write(directory, body: str):
    (directory / "agentinc.toml").write_text(body)
    return directory
    ...
    with pytest.raises(ManifestError, match="declare secret names only"):
        load_tool_manifest(
            _write(
                ...
                slug = "@acme/jira"
                version = "1.0.0"
                entry = "agentinc_tools.jira:search"
...
def test_tool_declares_config_and_secret_names(tmp_path):
    manifest = load_tool_manifest(
        _write(
            ...
            version = "1.4.2"
            kind = "cli"
            entry = "agentinc_tools.jira:search_issues"
